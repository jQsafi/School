												...
												<?php
					                                ...
					                                ?>
					                                    <option value="<?php echo $srow['exam_id'];?>"
					                                        <?php if($_POST['exam_id'] == $srow['exam_id'])echo 'selected';?>>
					                                            <?php echo $srow['name'];?>
					                                    </option>
					                                <?php
																?>
...
<table class="table" id="print">
	...
	<tr>
		...
		<td>
			...
			$condition['grade !=']='F';
			$total_passed=get_single_value('count(distinct(student_id))','exam_result',$condition);
			?>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			...
			foreach($students->result() as $student)
			{
				...
				$total_gpa=0;
				$exam_count=0;
				$condition_2['student_id']=$student_id;
				if($exam_id)
				{
					$condition_2['exam_id']=$exam_id;
					$condition_2['sub_exam_id']=$sub_exam_id;
				}
				$lg=get_single_value('grade','exam_result',$condition_2);
				if($lg=='A+')
				{
					$ap++;
				}
	...
	<?php
		...
		if($exam_id)
		{
			...
			$this->db->where('mark.sub_exam_id',$sub_exam_id);
		}
