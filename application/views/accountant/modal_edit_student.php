	...
	foreach($classes as $classt)
	{
		...
		foreach($groups as $group)
		{
			...
			foreach($subjects as $subject)
			{
				$subject_id=$subject->subject_id;
				$subject_namae=$subject->name;
				$str.='<li><input type="checkbox" class="fourthsubject" name="forthsubject[]" value='.$subject_id;
				if($fourth_subject==$subject_id)
				$str.=' checked="true"';
				$str.='>'.$subject_namae.'[foruth]</li>';
				/*else
		...
		foreach($subjects as $subject)
		{
			...
			if($fourth_subject==$subject_id)
			{
				...
				$nogroup_fourth_str.='>'.$subject_namae.'[foruth]</li>';
		}
...
<div class="tab-pane box active edit-student-container" id="edit" style="padding: 5px">
    <div class="box-content">
        <?php foreach ($edit_data as $row): ?>
            <?php echo form_open('accountant/student/' . $row['class_id']. '/do_update/' . $row['student_id'], array('class' => 'form-horizontal validatable', 'target' => '_top', 'enctype' => 'multipart/form-data')); ?>
            <div class="padded">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" class="" name="student_unique_ID"  value="<?=$row['student_unique_ID']?>" placeholder="XX-XX-XXX"/>
                    </div>
                </div>
                <div class="control-group">
                ...
                <div class="control-group custom-block">                                    
                    ...
                    <div class="controls">
						...
						<?=make_select('class_section','section_name','section_name',$row['section'])?>
						</select>
                ...
                <div class="control-group custom-block">
                    ...
                    <div class="controls">
						...
						<select name="academic_year" class="uniform validate[required]" style="width:100%;">
											...
											<?=make_select('academic_year','academic_year','academic_year',$row['academic_year'])?>
										</select>
                    </div>
					<div class="control-group">
									<?php
                ...
                <div class="control-group custom-block">
                    ...
                    <div class="controls">
						...
						<select name="others_section" class="uniform" style="width:100%;">
							<option value=""><?=translate('section')?></option>
							<?=make_select('class_section','section_name','section_name',$row['others_section'])?>
						</select>
...
/*$('input.forth-checkbox').on('change', function(evt) {
   ...
   if($(this).siblings(':checked').length >= forth) {
	   ...
	   {
	      /*var maxAllowed = 3;
		  var $v=$("input[name='mainsubject[]']");
	      var cnt = $v.filter(":checked").length;
	      if (cnt > maxAllowed)
	      {
	         $(this).prop("checked", "");
  ...
  $("input[name='forthsubject[]']").change(function () {
	      var maxAllowed = 1;
		  var $v=$("input[name='forthsubject[]']");
	      var fourth_subject_count = $v.filter(":checked").length;
	      if (fourth_subject_count > maxAllowed)
	      {
	         $(this).prop("checked", "");
