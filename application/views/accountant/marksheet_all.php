...
<?php
	...
	$mark_whole= merit_position('',$class_id,$position);
	$merit_str.='<td colspan=3>'.$position.'</td>';
	$total_str.='<td colspan=3>&nbsp;</td>';
	$total_str.='<tr>';
	...
	$over_all_total_grade=number_format($over_all_total_grade,2);
	$over_all_total_lg=get_letter_grade($over_all_total_grade);
	$gran_final_gpa=$gran_final_gpa/$exam_counter;
	$grand_final_lg=get_letter_grade($gran_final_gpa);
	foreach($subjects as $subject_id)
	{
		...
		foreach($result->result() as $row)
		{
			...
			if($subject_counter==0)
			{
				...
				$sub_exam_str.='<td rowspan='.$rowspan.'>'.$final_totals[$exam_id]['total'].'</td>';
				$sub_exam_str.='<td rowspan='.$rowspan.'>'.$final_exam_gpa.'</td>';
				$sub_exam_str.='<td rowspan='.$rowspan.'>'.get_letter_grade($final_exam_gpa).'</td>';
			}
		...
		{
			$sub_row_spn=$rowspan+2;
			$main_exam_str.='<td rowspan='.$rowspan.'>'.$over_all_total_marks.'</td>';
			$main_exam_str.='<td rowspan='.$rowspan.'>'.$over_all_total_grade.'</td>';
			$main_exam_str.='<td rowspan='.$rowspan.'>'.get_letter_grade($over_all_total_grade).'</td>';
		}
		$main_exam_str.='<tr>';
		$subject_counter++;
	}
