...
<html>
	...
	<?php $this->load->view('context_menu');?>
<div class="box box-border" id="main_body">
	...
	<div class="box-content padded textarea-problem">
            ...
            <div class="tab-pane  <?php if(!isset($edit_data) && !isset($personal_profile) && !isset($academic_result) )echo 'active';?>" id="list">
				<center>
                <?php echo form_open('accountant/marks');?>
                <table border="0" cellspacing="0" cellpadding="0" class="table table-normal box">
					...
					<tr>
					<td colspan="2">
						<?php echo form_open('accountant/full_mark');?>
						...
						<table class="table table-normal box">
							<thead>
								...
								<tr>
									...
									<td>
										<a href="<?=site_url('accountant/manage_attendance/'.$class_id.'/'.$exam_id.'/'.$sub_exam_id.'/'.$group_name)?>" class="btn btn-red" window="new" win_height="800px" win_width="800px">
                ...
                <?php 
						////CREATE THE MARK ENTRY ONLY IF NOT EXISTS////
							if($group_name)
							{
						    $students = $this->db->where(array('class_id' => $class_id,'group' => $group_name,'status'=>1))->order_by('roll')->get('student')->result_array();
							}
							else
								$students = $this->db->where(array('class_id' => $class_id,'status'=>1))->order_by('roll')->get('student')->result_array();
                ...
                <table class="table tablesorter">
                    <thead>
                        <tr>
                            ...
                            <th class="{sorter: false}"><?php echo translate('Written');?></th>
                            <th class="{sorter: false}"><?php echo translate('objective');?></th>
                            <th class="{sorter: false}"><?php echo translate('practical');?></th>
                            <th class="{sorter: false}"><?php echo translate('SBA');?></th>
							<th class="{sorter: false}"><?php echo translate('full_mark');?><br>
							<input type="checkbox" name="same" id="same" class="input" checked="true" style="width:10px;"/>
							All values are same?</th>
					...
					<tbody>
                        ...
                        <tr>
							...
							<button type="submit" class="btn btn-normal btn-gray "> Update Marks </button></td>
                        </tr>
						</tfoot>
                  </table>
