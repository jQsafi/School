<?php 
	...
	if($header_row_span_count==1) $header_row_span_count=2;
		$main_colspan_count=$sub_exam_count*11;
	$sub_exam_head_str='';
	$total_str='';
	$merit_str='';
	...
	if($sub_exams->num_rows()>0)
	{
		foreach($sub_exams->result() as $exam)
		{
			...
			$sub_exam_head_str.='<td colspan=11 class=sub_exam_name>'.$sub_exam_name.'</td>';
	...
	else
	{
		$mark_condition=array(
			...
			{
					$eighty    = $mark->total_mark * 0.8;
					$twenty    = $mark->total_mark * 0.2;
					...
					$total_str.='<td class=green_header>'.$mark->total_mark.'</td>';
					$total_str.='<td>'.$heigst.'</td>';
					$total_str.="<td>".$eighty."</td>";
					$total_str.="<td>".$twenty."</td>";
					$total_str.='<td>'.$mark->gpa.'</td>';
					$total_str.='<td class=final_gpa>'.$mark->grade.'</td>';
...
?>
		<table class="result-card">
			...
			<tr class="green_header">
				...
				{
				?>
				<td><?=translate('total_mark')?></td>
				<td><?=translate('GPA')?></td>
				<td><?=translate('latter_grade')?></td>
				<?php } ?>
			</tr>
			<?php 
				...
				<?php
					if($sub_exams->num_rows()>0)
					{
						...
						foreach($sub_exams->result() as $exam)
						{
							...
							$marks=$this->db->where('subject_id',$subject)->where('exam_id',$exam_id)->where('sub_exam_id',$sub_exam_id)->where('class_id',$class_id)->where('student_id',$student_id)->from('mark')->get();
							if($marks->num_rows()>0)
							{
								$exam_subject_count++;
					...
					{
					$marks=$this->db->where('subject_id',$subject)->where('exam_id',$exam_id)->where('sub_exam_id','99999')->where('class_id',$class_id)->where('student_id',$student_id)->from('mark')->get();
					if($marks->num_rows()>0)
					{
						foreach($marks->result() as $rows)
						{
