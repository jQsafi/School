...
<div class="box box-border">
    ...
    <input type="hidden" name="operation" value="update" />
        ...
        <?php for($day=1;$day<=$totalday;$day++){  ?>
          ...
          <?php
        $i=0;  
        $students = $this->db->get('teacher')->result_array();
        foreach($students as $row):  
         ?>
      
       <tr>
            <td rowspan="" >
                 <img src="<?php echo base_url();?>uploads/teacher_image/<?php echo $row['teacher_id'];  ?>.jpg" width="96" height="70" />
                Name: <?php echo $row['name']; ?>  Roll :<?php echo $row['roll']; ?>  </td>
            
            <td>
                     <?php     foreach($Finalsubjects as $eachsubid):
        $Allattendanceinfo=$this->db->get_where('attendance', array('month'=>$month,'year'=>$year,'student_id' => $row['techer_id']))->result_array();             
